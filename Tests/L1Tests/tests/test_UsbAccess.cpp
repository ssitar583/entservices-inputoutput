...
TEST_F(UsbAccessTest, getFileListSuccess_whenAbsPathFound)
{
    ...
    bool isCorrectPath = response.find("\"path\":\"\\/run\\/media\\/sda1\\/logs\\/PreviousLogs\"") != std::string::npos;
    EXPECT_TRUE(isCorrectPath);
    bool isFilePresent = response.find("{\"name\":\"logFile.txt\",\"t\":\"f\"}") != std::string::npos;
...
TEST_F(UsbAccessTest, getFileListSuccess_withoutPath)
{
    ...
    bool isCorrectPath = response.find("\"path\":\"\\/run\\/media\\/sda1\\/logs\"") != std::string::npos;
    EXPECT_TRUE(isCorrectPath);
    bool isFilePresent = response.find("{\"name\":\"PreviousLogs\",\"t\":\"d\"}") != std::string::npos;
    EXPECT_TRUE(isFilePresent);
    bool isSuccess = response.find("\"success\":true") != std::string::npos;
    EXPECT_TRUE(isSuccess);
...
TEST_F(UsbAccessEventTest, onArchiveLogsFailure_whenSetmntentValueNull)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onArchiveLogs"), _T("org.rdk.UsbAccess"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onArchiveLogs"), _T("org.rdk.UsbAccess"), message);
 }
    ...
    EXPECT_CALL(*p_wrapsImplMock, v_secure_popen(::testing::_,::testing::_, ::testing::_))
        ...
        .WillRepeatedly(::testing::Invoke(
            [&](const char *direction, const char *command, va_list args) {
                ...
                va_end(args2);
                EXPECT_EQ(string(strFmt), string(_T("/lib/rdk/usbLogUpload.sh /run/sda1")));
                return __real_popen(direction, command);
            }));
...
TEST_F(UsbAccessEventIarmTest, onUSBMountChangedSuccess)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onUSBMountChanged"), _T("org.rdk.UsbAccess"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onUSBMountChanged"), _T("org.rdk.UsbAccess"), message);
 }
