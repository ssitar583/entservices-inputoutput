...
# Testcase ID : TCID018_setpreferences_018_invalidDatatype
...
print("TC Description - Sets preferences for Front Panel LED indicators to invalid datatype which are saved to /opt/fp_service_preferences.json. This function neither validates an input nor changes LED states (color, brightness). It’s the users responsibility to provide valid and updated data.")
print("---------------------------------------------------------------------------------------------------------------------------")
...
curl_response = Utils.send_curl_command(FrontPanelApis.set_preferences_invalid)
if curl_response:
     Utils.info_log("curl command to set the preference is sent from the test runner")
else:
...
tc_id = 'TCID018_DS_FrontPanel_setPreferencesInvalidDatatype'
